...
export const settingsKeyboard = async(id: number) => {
    ...
    return new InlineKeyboard()
        .text(`${user.setting.notification_news ? `🔕 Выключить` : `🔔 Включить`} уведомления о новостях`, notificationNews.pack({
            news: !user.setting.notification_news
        }))
        .row()
        .text(`${user.setting.notification_schedule ? `🔕 Выключить` : `🔔 Включить`} уведомления о расписании`, notificationSchedule.pack({
