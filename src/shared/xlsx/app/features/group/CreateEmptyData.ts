...
export class CreateEmptyData {
    ...
    private filter(create: CreateMissingNumbers): number[][] {
        ...
        const keys_number = Object.keys(this.result[this.count]).map((res) => Number(res.split("__EMPTY_")[1]) || 0)

        this.result = create.CreateMissingNumbers(keys_number)
    ...
    public CreateEmptyData(create: CreateMissingNumbers): string[][] {
        this.result = this.filter(create).map(value =>
            value.map(res => res === null ? "__EMPTY" : "__EMPTY_" + res)
        );
